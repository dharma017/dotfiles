  ...
  //  fields:"",
   // dataValues:"",
    ...
    // pass=$("#txtPwd").val().trim();
      if(window.cordova){
        db = window.sqlitePlugin.openDatabase({name: "bip.sqlite",androidDatabaseImplementation:2,androidLockWorkaround: 1});
        sqlhelper.db = window.sqlitePlugin.openDatabase({name: "bip.sqlite",androidDatabaseImplementation:2,androidLockWorkaround: 1});  
      }else{
   ...
   //  console.log(fields);
    ...
    clearAllData: function(tables){
       if(window.sqlitePlugin!=undefined){
          localStorage.clear();
          $.jStorage.flush();
          window.sqlitePlugin.deleteDatabase("bip.sqlite");
