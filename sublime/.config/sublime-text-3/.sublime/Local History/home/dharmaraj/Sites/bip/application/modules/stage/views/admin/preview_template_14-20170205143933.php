...
<div id ="contentArea" class="pad10 row clear previewTemplate14">
    <?php
    if($description)
    ...
    <div class="formentry11">
        <form>
            ...
            foreach($chunkedTemplateData as $key => $innerChunk){
                echo '<div class="custom_template worksheet template_10 divider_'.$key.'">';
                foreach ($innerChunk as $key1 => $rows) {
                        ...
                        echo '<div class="groupWrapper" style="width: '.$width.'%; margin-left:'.$user_margin.'%;"><div class="txt_area'.$cnt.' headline">'.$rows->fld_label.'</div>';
                        echo '<div class="grp_txt_area">';
                        for($i = 0; $i < $row_in_working_sheet; $i++){
                            $count = $i*10+1+$key1;
                            echo '<div class="txt_area'.$cnt.'"><textarea class="form_texarea" rows="'.$row_in_text_field.'" style="width:100%"></textarea></div>';
                        }
                        echo '</div></div>';
                }
                echo '</div>';
