...
class user_model extends CI_Model {
	...
	function getAllUser($offset=0, $datalimit=50, $orderBy='', $psychologistid) {  // order by is no more  used in query. it is feed by session
		...
		$strSql .= ' AND b.is_deleted=0 AND b.lang_id= '.$language_code;
	...
	function getAllUser222($offset=0, $datalimit=50, $orderBy) {
			...
			//  echo $psychlogistId;exit;
				...
				if($grp_multi){
					foreach ($permission['rights_per_group'] as $key => $diff_data) {
						...
						if($permission['rights_per_group'][$key]['create_psychologists'] == 1){
							if($where!="("){
									$where.=" OR ";
								}
								$where .= "(b.created_by = ".$psychlogistId.")   ";
							}
		...
		if ($psychologist_id || $difficulty_id || $group_id) {
			...
			if ($difficulty_id && !is_array($difficulty_id)){
			 ...
			 $where2 = "  FIND_IN_SET($difficulty_id,b.difficulty_id) ";
			 ...
			 if ($group_id && !is_array($group_id)){
			 	...
			 	$where2 = "  FIND_IN_SET($group_id,b.group_id) ";
...
function getSelectListGrp(){
	...
	function addUser() {
		...
		'psycho_manage' => (int) $this->input->post('psycho_manage')
			...
			$group_id = $this->input->post('new_group_id');
			 //dd($_POST['group_id']);
			//echo '<pre>';print_r($group_id);
			$difficulty_id = $this->input->post('new_difficulty_id');
		...
		if ($user_role == 1)
			...
			$group_keys = array_keys($rights_per_group_arr);
			$group_db = implode(',',$group_keys);

			$difficulty_keys = array_keys($rights_per_difficulty_arr);
			$difficulty_db = implode(',',$difficulty_keys);
		}
