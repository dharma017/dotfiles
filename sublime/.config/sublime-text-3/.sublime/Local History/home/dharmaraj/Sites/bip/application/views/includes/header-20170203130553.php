					    ...
					    $.ajaxSetup({
					        error: function (x, status, error) {
					            if (x.status == 403) {
					                ...
					                window.location.href ="/index.php/user/error_report";
					            }
					        }
					    });
					});
	...
	<script type="text/javascript">
		...
		$(function () {
			...
			if (isWorksheet) {
				...
				$('#update_worksheet,#update_worksheet_top').on('click',function (e) {
					worksheetDefaultForm = $worksheet_form.serialize();
					checkIfFormChanged();
				});
		...
		function unloadPage(link) {
			...
			$.confirm({
		                        'title': "You have unsaved changes on this page. Do you want to leave this page and discard your changes or stay on this page?",
		                        'message': '',
		                        'buttons': {
		                            'yes': {
		                                'class': 'blue',
