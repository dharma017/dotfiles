...
class Todo extends CI_Controller {
	...
	/**
		...
		php index.php user todo fix_user_permission
	 */
	...
	function prepare_test_site(){
		...
		// fix internal links
		$this->db->query("UPDATE `bip_step` SET `description` = replace(description, 'https://barninternetprojektet.se', 'http://bip.local')");
		echo $this->db->last_query();echo "\n\n";

		$this->db->query("UPDATE `bip_user_tracking` SET `exit_page` = replace(exit_page, 'https://barninternetprojektet.se', 'http://bip.local')");
		echo $this->db->last_query();echo "\n\n";
	...
	function prepare_live_site(){
		...
		// fix internal links
		$this->db->query("UPDATE `bip_step` SET `description` = replace(description, 'https://barninternetprojektet.se', 'https://bip.websearchpro.net')");
		echo $this->db->last_query();echo "\n\n";

		$this->db->query("UPDATE `bip_user_tracking` SET `exit_page` = replace(exit_page, 'https://barninternetprojektet.se', 'https://bip.websearchpro.net')");
		echo $this->db->last_query();echo "\n\n";
	...
	/**
	 UPDATE bip_admin_user set sms_login=0,contact_number='';
	UPDATE bip_user set sms_notify=0,email_notify=0,email='',contact_number='',active_to='2022-05-31';
	UPDATE bip_admin_user SET pass='$2a$08$PoFyOuw2WyXv.m/sk4yPYeVcQPOKlIBlLSwGNMMqFUdvUdmpn7whK';
	UPDATE bip_user SET password='$2a$08$PoFyOuw2WyXv.m/sk4yPYeVcQPOKlIBlLSwGNMMqFUdvUdmpn7whK';
	...
	function get_psychologist_groups($id)
	{
		$group_list = array();
		$groups =  $this->db->query("SELECT distinct group_id from bip_user where user_role = 1 and psychologist_id = $id and is_deleted=0")->result();
	...
	function fix_user_permission()
	{
		if (!is_cli()) return;

		$psychologists = $this->db->query("SELECT `id`,`username`,`permission` FROM `bip_user` WHERE `user_role` = '2' and  is_deleted=0")->result();

		$rights_per_group = array(
			'treat_patients'=>1,
			'manage_users'=>0,
			'create_psychologists'=>0,
			'manage_psychologists'=>0,
			'extract_data'=>0
			);

		$rights_per_difficulty = array(
			'edit_difficulty'=>1,
			'give_rights_to_edit_difficulty'=>0
			);

		$other_rights  = array(
			'create_new_difficulty'=>0,
			'create_new_group'=>0
			);
		...
		foreach ($psychologists as $psychologist) {
			...
			$old_groups = $this->get_psychologist_groups($psychologist->id);

			if (!empty($old_groups)) {
				$group_join = implode(',',$old_groups);
				$this->db->query("UPDATE bip_user SET group_id= ? WHERE id= ?",array($group_join,$psychologist->id));
			}

			foreach ($old_groups as $group) {
				$psychologist_data[$psychologist->id]['rights_per_group'][$group] = $rights_per_group;
			}

			$old_permission = json_decode($psychologist->permission);

			foreach ($old_permission->diff_multi as $key => $difficulty) {
				$psychologist_data[$psychologist->id]['rights_per_difficulty'][$difficulty] = $rights_per_difficulty;
			}

			$psychologist_data[$psychologist->id]['other_rights'] = $other_rights;
		}
		// dd($psychologist_data);

		foreach ($psychologist_data as $psychologist_id => $psychologist_arr) {
			$permission_json = json_encode($psychologist_arr);
			$this->db->query("UPDATE bip_user SET permission= ? WHERE id= ?",array($permission_json,$psychologist_id));
			echo $this->db->last_query();echo "\n\n";
		}
		$this->fix_psychologist_difficulty();

	}

	function fix_psychologist_difficulty()
	{
		if (!is_cli()) return;
		...
		foreach ($psychologists as $psychologist) {
				$permission_json = json_decode($psychologist->permission,true);
				$rights_per_difficulty = $permission_json['rights_per_difficulty'];
				$rights_per_difficulty_keys = array_keys($rights_per_difficulty);
				if (!empty($rights_per_difficulty_keys)) {
					$difficulty_join = implode(',',$rights_per_difficulty_keys);
					$this->db->query("UPDATE bip_user SET difficulty_id= ? WHERE id= ?",array($difficulty_join,$psychologist->id));
