...
var sqlhelper = {
    ...
    initiateDatabase:function(response,success){
      ...
      // if(window.cordova && IsIDevice){
        sqlhelper.db = window.sqlitePlugin.openDatabase({name: "bip.sqlite", key: '4h%v!hMJiXgK', location: 'default'});  
      // }else{
        ...
        if(tables.length>0){
            sqlhelper.db.transaction(function(tx) {
                for(var k=0; k<tables.length;k++){
                    tx.executeSql("DROP TABLE IF EXISTS "+tables[k]+";", [], function(){
                      console.info('drop table '+tables['k']);
                    }, sqlhelper.errorCB);
                }
